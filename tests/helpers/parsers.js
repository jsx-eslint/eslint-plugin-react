...
const oldBabelParser = require('babel-eslint');
const newBabelParser = require('@babel/eslint-parser');
const oldTSParser = require('typescript-eslint-parser');
const newTSParser = require('@typescript-eslint/parser');
...
function minEcmaVersion(features, languageOptions) {
  ...
  const result = Math.max.apply(
    ...
    []
      .concat(
        (languageOptions && languageOptions.ecmaVersion) || [],
...
const parsers = {
  BABEL_ESLINT: oldBabelParser,
  '@BABEL_ESLINT': newBabelParser,
  TYPESCRIPT_ESLINT: oldTSParser,
  '@TYPESCRIPT_ESLINT': newTSParser,
  disableNewTS,
  ...
  babelParserOptions: function parserOptions(test, features) {
    return Object.assign({}, (test.languageOptions && test.languageOptions.parserOptions) || {}, {
      ...
      ecmaFeatures: Object.assign(
        {},
        test.languageOptions && test.languageOptions.ecmaFeatures,
        {
          jsx: true,
          modules: true,
  ...
  all: function all(tests) {
    const t = flatMap(tests, (test) => {
      ...
      const es = minEcmaVersion(features, test.languageOptions);

      function addComment(testObject, parser) {
        const extras = [].concat(
          `features: [${Array.from(features).join(',')}]`,
          `parser: ${parser}`,
          testObject.languageOptions ? `languageOptions: ${JSON.stringify(testObject.languageOptions)}` : [],
        ...
        skipBase
          ...
          : addComment(
            Object.assign(
              ...
              test,
              typeof es === 'number' && {
                languageOptions: Object.assign({}, test.languageOptions, {
                  ecmaVersion: es,
                }),
              }
            ),
