 ...
 */
function inLifeCycleMethod(context, checkAsyncSafeLifeCycles, node) {
...
function isPropTypesUsageByMemberExpression(node, context, utils, checkAsyncSafeLifeCycles) {
  ...
  if (isInClassComponent(context, node)) {
    ...
    if (
      ...
      && (inLifeCycleMethod(context, checkAsyncSafeLifeCycles, node) || astUtil.inConstructor(context, node))
    ) {
      return true;
    }
    // this.setState((_, props) => props.*))
    if (isPropArgumentInSetStateUpdater(context, unwrappedObjectNode)) {
      return true;
    }
    return false;
  }
