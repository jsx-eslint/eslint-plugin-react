...
/**
 * Get the root parent of a given AST node
 * For example, the root parent of `foo.bar.qux` is `foo`.
 * @param {ASTNode} node The AST node being checked.
 */
function getRootParentName(node) {
  if (node && node.object) {
    return node.object.name || getRootParentName(node.object);
  }
...
function getDeclarationType(initExpression) {
  ...
  if (initExpression.type === 'CallExpression' && initExpression.callee.type === 'CallExpression') {
...
module.exports = {
  ...
  getRootParentName: getRootParentName,
