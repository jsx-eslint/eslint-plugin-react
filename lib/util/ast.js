...
/**
 * Check whether node matches with member string
 *
 * @param {ASTNode} node The AST node being checked
 * @param {String} str The member string The AST node be checked against.
 */
function isMemberExpressionPrintedTo(node, str) {
  const strNames = str.split('.');

  let expressionNode = node;
  let i = strNames.length - 1;
  for (; i > 0; i--) {
    if (
      expressionNode.type !== 'MemberExpression'
      || expressionNode.property.name !== strNames[i]
  ...
  return expressionNode.type === 'Identifier'
    && expressionNode.name === strNames[0];
}
...
module.exports = {
  ...
  isMemberExpressionPrintedTo: isMemberExpressionPrintedTo,
