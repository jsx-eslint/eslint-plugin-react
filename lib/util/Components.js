...
function componentRule(rule, context) {
  ...
  const utils = {
    ...
    isES5Component: function(node) {
      ...
      return new RegExp(`^(${localName}\\.)?${createClass}$`).test(sourceCode.getText(node.parent.callee));
    },
    ...
    isES6Component: function(node) {
      ...
      return new RegExp(`^(${localName}\\.)?(Pure)?Component$`).test(sourceCode.getText(node.superClass));
    },
    ...
    isPureComponent: function (node) {
      if (node.superClass) {
        return new RegExp(`^(${localName}\\.)?PureComponent$`).test(sourceCode.getText(node.superClass));
      }
      return false;
    },
     ...
     *
     * @todo Fix this to match with the implementation of `isJsxPragma` in
     * button-has-type rule.
     *
     * @param {ASTNode} node The AST node being checked.
    ...
    isCreateElement: function(node) {
      const calledOnPragma = (
        ...
        node.callee &&
        node.callee.object &&
        node.callee.object.name === localName &&
    ...
    isPragmaComponentWrapper(node) {
      ...
      if (calleeObject && node.callee.property) {
        return arrayIncludes(propertyNames, node.callee.property.name) && calleeObject.name === localName;
      }
