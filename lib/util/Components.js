...
function componentRule(rule, context) {
  ...
  const utils = {
     ...
     * @param {ASTNode} node The node to check its name
     * @returns {Boolean} True if createElement is destructured from the pragma
     */
    isDestructuredFromPragmaImport(variable, node) {
      return isDestructuredFromPragmaImport(variable, context, node);
    },
      ...
      return wrapperFunctions.some((wrapperFunction) => {
        ...
        return wrapperFunction.property === node.callee.name
          && (!wrapperFunction.object
            ...
            || (wrapperFunction.object === pragma && this.isDestructuredFromPragmaImport(node.callee.name, node))
          );
      });
    },
     ...
     * @returns {ASTNode} component node, null if we are not in a component
     */
    getParentStatelessComponent(_node) {
    ...
    getRelatedComponent(node) {
      ...
      let variableInScope;
      const variables = variableUtil.variablesInScope(context, initalNode);
    ...
    isReactHookCall(node, expectedHookNames) {
      ...
      const scope = (isPotentialReactHookCall || isPotentialHookCall) && (context.sourceCode || context).getScope(node);
