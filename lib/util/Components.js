...
Components.prototype.list = function() {
  ...
  for (const j in this._list) {
    ...
    const nodeJ = this._list[j].node;
    for (const k in this._list) {
      if (!has(this._list, k)) {
        continue;
      }
      const nodeK = this._list[k].node;
      if (nodeJ.start > nodeK.start && nodeJ.end < nodeK.end && this._list[j].confidence > 1) {
        this._list[k].usedPropTypes = mergeUsedPropTypes(
          this._list[j].usedPropTypes || [],
          this._list[k].usedPropTypes || []
        );
      }
    }
  }
