...
module.exports = function propTypesInstructions(context, components, utils) {
  ...
  function resolveValueForIdentifierNode(node, callback) {
    if (
      ...
      do {
        vars.variables.forEach((v) => allVars.push(v));
        vars = vars.upper;
      } while (vars);
      const identVariable = allVars.find((v) => v.defs.length && v.name === node.name);
  ...
  function markPropTypesAsDeclared(node, propTypes) {
    ...
    switch (propTypes && propTypes.type) {
      ...
      case 'Identifier': {
        const variablesInScope = variableUtil.variablesInScope(context, node);
