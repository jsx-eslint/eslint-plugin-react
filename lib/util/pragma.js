...
function getLocalNameFromContext(context) {
  let localName = 'React';

  // .eslintrc shared settings (http://eslint.org/docs/user-guide/configuring#adding-shared-settings)
  if (context.settings.react && context.settings.react.localName) {
    localName = context.settings.react.localName;
  } else if (context.settings.react && context.settings.react.pragma) {
    localName = context.settings.react.pragma;
  }

  if (!JS_IDENTIFIER_OR_MEMBER_REGEX.test(localName)) {
    throw new Error(`Local name ${localName} for React is not a valid identifier`);
  }
  return localName;
}
...
module.exports = {
  ...
  getLocalNameFromContext: getLocalNameFromContext,
