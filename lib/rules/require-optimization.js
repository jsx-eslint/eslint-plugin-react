...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    const isShouldComponentUpdate = () => {
      const blockNode = context.getScope().block;

      return blockNode &&
          blockNode.parent &&
          blockNode.parent.key &&
          blockNode.parent.key.name === 'shouldComponentUpdate';
    };

    return {
      ArrowFunctionExpression(node) {
        // Skip if the function is declared in the class or function is SCU itself
        if (isFunctionInClass() && !isShouldComponentUpdate()) {
          return;
        }
