  ...
  create(context) {
    ...
    */
    ...
    function validateBraceSpacing(node) {
      ...
      let third = context.getTokenAfter(second, { includeComments: true });
      let penpenultimate = sourceCode.getTokenBefore(penultimate, { includeComments: true });

      if (!third) {
        third = context.getTokenAfter(second);
        const leadingComments = sourceCode.getNodeByRangeIndex(third.range[0]).leadingComments;
        third = leadingComments ? leadingComments[0] : third;
      }
      if (!penpenultimate) {
        penpenultimate = sourceCode.getTokenBefore(penultimate);
        const trailingComments = sourceCode.getNodeByRangeIndex(penpenultimate.range[0]).trailingComments;
        penpenultimate = trailingComments ? trailingComments[trailingComments.length - 1] : penpenultimate;
      }

      const isObjectLiteral = first.value === second.value;
      const isArrayLiteral = second.value === '[' && penultimate.value === ']';
	  const isGrouping = second.value === '(' && penultimate.value === ')';
      const spacing = isObjectLiteral || isArrayLiteral || isGrouping ? config.objectLiteralSpaces : config.when;
      if (spacing === SPACING.always) {
      ...
      if (isObjectLiteral || isArrayLiteral) {
        ...
        if (!sourceCode.isSpaceBetweenTokens(penpenultimate, penultimate)) {
