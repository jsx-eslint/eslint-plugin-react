...
const messages = {
  ...
  addKeyProp: 'Add missing "key" prop',
};
...
module.exports = {
  ...
  create(context) {
    ...
    function makeAddKeyPropSuggestion(jsxElement) {
      return [
        Object.assign(
          getMessageData('addKeyProp', messages.addKeyProp),
          {
            fix(fixer) {
              const openingEl = jsxElement.openingElement;
              if (openingEl.attributes.length > 0) {
                return fixer.insertTextBefore(openingEl.attributes[0], 'key={false} ');
              }
              return fixer.insertTextAfter(openingEl.name, ' key={false}');
            },
          }
        ),
      ];
      ...
      if (node.type === 'JSXElement') {
        if (!hasProp(node.openingElement.attributes, 'key')) {
          report(context, messages.missingIterKey, 'missingIterKey', {
            node,
            suggest: makeAddKeyPropSuggestion(node),
          });
        } else {
          ...
          if (keys.length === 0) {
            if (node.type === 'ArrayExpression') {
              report(context, messages.missingArrayKey, 'missingArrayKey', {
                node: element,
                suggest: makeAddKeyPropSuggestion(element),
              });
            }
          } else {
            keys.forEach((attr) => {
