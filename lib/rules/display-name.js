...
module.exports = Components.detect(function(context, components, utils) {
  ...
  var acceptTranspilerName = config.acceptTranspilerName || config.forbidExplicitDefinition || false;
  var forbidExplicitDefinition = config.forbidExplicitDefinition || false;

  var MISSING_MESSAGE = 'Component definition is missing display name';
  var FORBIDDEN_MESSAGE = 'Explicit display name definition is forbidden';
  ...
  /**
   * Reports an explicitly defined display name for a given component
   * @param {Object} component The component to process
   */
  function reportExplicitDefinition(component) {
    context.report(
      component.node,
      FORBIDDEN_MESSAGE, {
        component: component.name
  ...
  return {
    ...
    'Program:exit': function() {
      ...
      for (var component in list) {
        if (forbidExplicitDefinition && list[component].hasDisplayName) {
          reportExplicitDefinition(list[component]);
        } else if (!list.hasOwnProperty(component) || list[component].hasDisplayName) {
          continue;
        } else {
