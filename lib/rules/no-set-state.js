...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    // function reportUsageOfSetState(node) {
    //   context.report({
    //     node,
    ...
    function reportSetStateUsages(component) {
      let setStateUsage;
      let calleeName;
      const calleeNames = component.setStateUsages && component.setStateUsages.map(node => getMethodName(node));
      for (let i = 0, j = component.setStateUsages.length; i < j; i++) {
        calleeName = calleeNames[i];
        setStateUsage = component.setStateUsages[i];
        const reportUsage = notAllowedIn.size ? notAllowedIn.has(calleeName) : true;
        if (reportUsage) {
          context.report({
            node: setStateUsage,
            message: 'Do not use setState'
