...
module.exports = {
  meta: {
    ...
    schema: [{
      ...
      properties: {
        ...
        allowArrayMutation: {
          description: 'If true, will allow all array mutations (e.g. `this.props.list.push(1)`). If an array, specify the allowable array mutations. https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array#Mutator_methods',
          type: ['boolean', 'array'],
          default: false
        }
      },
  ...
  create: Components.detect((context, components, utils) => {
    ...
    const allowArrayMutation = options.allowArrayMutation;
    const allowableArrayMutations = Array.isArray(allowArrayMutation)
      ? ARRAY_MUTATIONS.filter(mutation => allowArrayMutation.indexOf(mutation) < 0)
      : ARRAY_MUTATIONS;
    const enableArrayMutation = Array.isArray(allowArrayMutation)
      ? !allowArrayMutation.length
      : allowArrayMutation;
    ...
    return {
      ...
      CallExpression: function(node) {
        ...
        if (!isObjectMutation && !(isArrayMutation && !enableArrayMutation)) {
          return;
        }
