...
const isSpaceBetweenTokens = eslintUtil.isSpaceBetweenTokens;
...
function validateClosingSlash(context, node, option) {
  ...
  if (node.selfClosing) {
    ...
    adjacent = !isSpaceBetweenTokens(context, lastTokens[0], lastTokens[1]);
    ...
    adjacent = !isSpaceBetweenTokens(context, firstTokens[0], firstTokens[1]);
...
function validateBeforeSelfClosing(context, node, option) {
  ...
  const adjacent = !isSpaceBetweenTokens(context, leftToken, closingSlash);
...
function validateAfterOpening(context, node, option) {
  ...
  const adjacent = !isSpaceBetweenTokens(context, openingToken, node.name);
...
function validateBeforeClosing(context, node, option) {
  ...
  if (!node.selfClosing) {
    ...
    const adjacent = !isSpaceBetweenTokens(context, leftToken, closingToken);
