    ...
    schema: [
      {
        type: 'object',
        properties: {
          defaultAllowOnDOM: {type: 'boolean'},
          defaultAllowOnComponents: {type: 'boolean'},
          forbid: {
            type: 'array',
            items: {
              oneOf: [
                {type: 'string'},
                {
                  ...
                  properties: {
                    ...
                    allowOnDOM: {type: 'boolean'},
                    allowOnComponents: {type: 'boolean'},
                    forbid: schemaStringArray,
                    allow: schemaStringArray
                  }
                }
              ]
            }
  ...
  create: function(context) {
    ...
    function getError(prop, tag) {
      if (rules[prop].forbid.indexOf(tag) > -1) {
        return 'Prop `' + prop + '` is specifically forbidden on ' + tag +
            (isDOMNode(tag) ? ' DOM nodes' : ' Components');
