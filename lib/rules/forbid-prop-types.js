      ...
      VariableDeclaration: function(node) {
        node.declarations.forEach(declaration => {
          ...
          const requiredFromReact =
            ...
            && [REACT_MODULE, PROP_TYPES_MODULE].includes(declaration.init.arguments[0].value);

          if (isRequire && !requiredFromReact) {
            customImports.push(declaration.id.name);
          }
        });
      },

      ImportDeclaration: function(node) {
        const importedFromReact = node && node.source && [REACT_MODULE, PROP_TYPES_MODULE].includes(node.source.value);
